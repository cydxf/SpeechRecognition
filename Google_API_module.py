...
def transcribe_audio_segment(audio_segment):
    ...
    参数:
    audio_segment (AudioSegment): 音频片段

    返回:
    str: 转换后的文本
    """
    recognizer = sr.Recognizer()
    audio_path = "temp_segment.wav"
...
def transcribe_long_audio(file_path):
    ...
    segment_length = 30000  # 30秒
    segments = [audio[i:i + segment_length] for i in range(0, len(audio), segment_length)]

    transcription = ""
    for i, segment in enumerate(tqdm(segments, desc="转换音频片段")):
        transcription += transcribe_audio_segment(segment) + " "
